...
private:
    ...
    // Status-bar battery gauge (#260/#261). Only compiled on boards whose
    // profile sets hasBattery -- a USB-only board carries none of this.
    bool ensure_device_service_binding();
    void release_device_service_binding();
    void refresh_battery_status();
    void set_status_battery_state(bool present, int percentage, bool charging, bool low, bool critical);
#endif
...
#if defined(BOARD_HAS_BATTERY) && BOARD_HAS_BATTERY
    service::ServiceBinding device_service_binding_;  // Device service, for battery state (#261)
#endif
