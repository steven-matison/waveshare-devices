...
def write_launcher_cmake(work, prof):
    """Generated main/board_profile.cmake -> per-board flags as GLOBAL compile
    definitions. Consumers live in the brookesia_system_super component (the
    launcher; the status bar) with no include path back to main/, so a header
    cannot reach them; a build-wide compile definition does. Included (OPTIONAL)
    by examples/system/super/CMakeLists.txt. Always writes a definite 0/1 so the
    consuming #if is deterministic.
    ...
    body = (
        ...
        f"idf_build_set_property(COMPILE_DEFINITIONS \"BOARD_HAS_BATTERY={battery}\" APPEND)\n"
    )
    open(p, "w").write(body)
